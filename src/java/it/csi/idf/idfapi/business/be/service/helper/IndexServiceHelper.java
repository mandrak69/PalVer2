...
public class IndexServiceHelper {
	...
	public String indexGetFolder(MetadatiIndexPfa metadati) throws ServiceException {
		...
		try {
			...
			if (metadati.isNotifica()) {
				...
				luceneQueryIdIntervento.append(Constants.INDEX_FOLDER_SUFFIX);
				luceneQueryIdIntervento.append(metadati.getIdIntervento());

				uidFolder = this.indexSearchFolder(
						IndexDtoFactory.createSearchParamsForIndexFolder(luceneQueryIdIntervento.toString()),
						metadati.getIdIntervento(), uidFolderIdPfa);
				...
				luceneQueryFolderIntervento.append(Constants.INDEX_FOLDER_SUFFIX);
				luceneQueryFolderIntervento.append(MetadatiIndexPfa.FOLDER_INTERVENTI);

				this.indexSearchFolder(
						IndexDtoFactory.createSearchParamsForIndexFolder(luceneQueryFolderIntervento.toString()),
	...
	public Content downFile(String uidAllegato, MetadatiIndexTrasformazioni metadati) throws ServiceException {
		byte[] result = null;
		try {

			byte[] res=this.indexDownloadFileTrasformazioni("sada120eura.jpg");
			final OperationContext oc = this.indexGetOperationContext(Constants.INDEX_USERNAME_ADMIN);
			result = this.ecmEngineWebServiceDelegate.retrieveContentData(IndexDtoFactory.createNode(uidAllegato),
					IndexDtoFactory.createContent(), oc);
			
			Content resCont = this.indexCreateContent(metadati, "slika456.jpg", result);
			return resCont;
		} catch (Exception e) {
			LOGGER.error("[IndexServiceHelper::indexDownloadFile] ERROR GENERICO: " + e.getMessage(), e);
			throw new ServiceException("[Errore INDEX indexDownloadFile Exception] " + e.getMessage());
