...
public class EventoPartforDAOImpl implements EventoPartforDAO {
	...
	@Override
	public void updateEventoPartfor(Integer idEvento, Integer idGeoPlParticellaForest, Integer percDanno) {
		StringBuilder sql = new StringBuilder();
		
		sql.append("UPDATE idf_r_evento_partfor SET perc_danno = ?");
		sql.append(" WHERE id_evento = ? AND idgeo_pl_particella_forest = ?");
		
		List<Object>parameters = new ArrayList<>();
		
		parameters.add(percDanno);
		parameters.add(idEvento);
		parameters.add(idGeoPlParticellaForest);
